...
const { isAuthenticated, ownsServer, PANEL_URL, API_KEY, sendCommandAndGetResponse } = require("./core.js");
...
module.exports.load = async function (app, db) {
  ...
  router.post("/server/:id/power", isAuthenticated, ownsServer, validate(schemas.serverPower), async (req, res) => {
    try {
      ...
      if (response.status === 204) {
        await recordServerActivity(db, req, serverId, `server.${signal}`, {
          signal,
        });
        res.status(204).send();
      } else {
  ...
  router.post("/server/:id/command", isAuthenticated, ownsServer, validate(schemas.serverCommand), async (req, res) => {
    try {
      ...
      await recordServerActivity(db, req, serverId, 'server.command', {
        command,
      });
